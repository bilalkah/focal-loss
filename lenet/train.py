...
tensor_y = torch.Tensor(np.load("mnist-target.npy")).type(torch.int)
...
zero_y = torch.Tensor(np.load("mnist-zero-target.npy")).type(torch.int)
...
zero_y = torch.Tensor(np.load("mnist-zero-test-target.npy")).type(torch.int)
zero_x = zero_x.permute(0,3,1,2)
zero_x = transforms.functional.resize(zero_x,(32,32))
print(zero_x.shape)
print(zero_y.shape)
